...
def worker(fin, fout_name, tmp_dir):
    ...
    for g in group:
        ...
        previous_index = 0
        size_param = 40
        window_size = min(size_param, len(cut_mrna))
        pervious_MEF_duplex = float('inf')
        new_row = pd.Series()
        for window in range(window_size, len(cut_mrna) + size_param, size_param):
            global_count += 1
...
def worker2(fin, fout_name, tmp_dir):
    print("##################NEW FILE#################################")
    print(fin)
    fout = filename_suffix_append(fout_name, "_negative")
    # fout = fout_name
    ...
    for g in group:
        ...
        mrna_cut = rows_group.iloc[0]["sequence"]
        mirna_seq = rows_group.iloc[0]['miRNA sequence']
        ...
        # search valid interaction on the site
        valid, properties = generate_negative_seq_2(mirna_seq, mrna_cut)

        new_row = pd.Series()
        if len(properties) != 0:
            new_row['paper name'] = rows_group.iloc[0]['paper name']
            new_row['organism'] = rows_group.iloc[0]['organism']
            new_row['miRNA ID'] = rows_group.iloc[0]['miRNA_ID']
            new_row['miRNA sequence'] = properties["mock_mirna"]
            new_row['Gene_ID'] = rows_group.iloc[0]['Gene_ID']
            new_row['full_mrna'] = row.sequence
    ...
    f = "/sise/home/efrco/efrco-master/generate_interactions/non_overlapping_sites/count.csv"
